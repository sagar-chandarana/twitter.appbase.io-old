					...
					selfRef.uuidPro().then(function(baseUuid){
						...
						for (var linkCollection in obj.links){
							for (var linkKey in obj.links[linkCollection]){
								pros[i] = ab.util.getTreePro(levels-1,AppbaseOld.ref(selfRef.getPath()+'/'+linkCollection+':'+linkKey,true));
...
AppbaseObj.prototype.addLink = function(linkName,uuid,noFire,order){
	...
	*/
	...
	if(!noFire)
		this.fire('link_added',AppbaseOld.ref(ab.util.parseCollectionFromPath(linkName)+":"+uuid))
	...
	switch(event){
		case 'value_changed':
			for (var id in ab.store.obj.parent.getParents(this.id)){
				ab.store.obj.parent.getParents(this.id)[id].parent.fire('link_changed',AppbaseOld.ref(this.collection+':'+this.id))
			}
		break;
		case  'link_changed':
		case  'link_added':
...
AppbaseObj.prototype.removeLink = function(linkName){
	...
	*/
	...
	this.fire('link_removed',AppbaseOld.ref(ab.util.parseCollectionFromPath(linkName)+":"+uuid))
}
...
AppbaseOld.ref = function(arg,dontFetch){
	return new AppbaseRef(arg,dontFetch);
}

AppbaseOld.new = function(arg){
...
//abR = new AppbaseOld.ref.('User:sagar/Tweet:ABC');
...
ab21 = new AppbaseOld.ref.('User:faad');


abR.addLink('User',ab21.getPath());

ab21.addLink('Tweet',(new AppbaseOld.ref.('Tweet:123')).getPath())


abRNew = new AppbaseOld.ref.('User:sagar/User:faad');


ab3 = new AppbaseOld.ref.('Tweet:123')
...
/*
abR = new AppbaseOld.ref.('User:sagar');

abR.on('link_removed',function(obj){
	console.log(obj);
})

abc =  new AppbaseOld.ref.('User:faad');
...
window['abFuncs'] = {};
AppbaseOld.search = function( query, dn) {
		...
		return function(uuids){	
			...
			Promise.all(uuids.map(function (uuid) {  return ab.store.obj.get.nowPro(uuid,false)})).then(function(objs){
				...
				objs.forEach(function(obj){
					refArray.push(AppbaseOld.ref(obj.collection+':'+obj.id));
				});
				dn(refArray);
			});
		}
...
/*
abR = AppbaseOld.ref('User:sagar');
abR.getTree(2);

abR.on('subtree_changed',function(obj){console.log(obj)});
abR.addLink(AppbaseOld.ref('Tweet:ABC',true));
AppbaseOld.ref('Tweet:ABC',true).set('asda','asdsd');
ab2 = AppbaseOld.ref('Tweet:yo'); //ref turns invalid after removing link??
ab2.set('ads','asfasc');
...
for(var i = 0;i<10;i++){
	new AppbaseOld.new('User:ad3'+i).set('yo',i)
}

AppbaseOld.search({collection:'User',sstring:'happy',limit:10},function(a){
	//console.log(a);
